...
For each prediction week, the final model:
...
The challenge requires the model to be tested on gateways it has not seen, tested on weeks after training, and evaluated under network changes.
...
# Execution and Reproducibility Decision
...
1. Locates the challenge data in the project's `data\` directory when available.
2. Requests the data path if the expected project data directory is not available.
3. Runs the final Machine Learning pipeline.
4. Generates `predictions.csv`.
5. Runs the submission validator.
6. Reports whether the generated predictions pass validation.

The individual Python scripts remain available so that specific components can also be executed separately for testing, validation, or debugging.

This approach keeps the execution simple while preserving transparency and reproducibility.
