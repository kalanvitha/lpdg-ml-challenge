...
- excludes the most recent **7 days**
- scores the following **7-day recent period**

Conceptually:

```text
Historical period                  Recent scoring period
<--------------------------->      <---------------->
        28 days                         7 days
        training                       prediction
```
...
1. **It is not trained on the official hidden challenge labels.**  
   The engineer-review information available during development is only proxy evidence.

2. **It uses only three telemetry metrics.**  
   Other telemetry signals may contain additional information about gateway failures.

3. **Isolation Forest identifies unusual behavior; it does not understand the physical cause of a failure.**  
   A high score means anomalous behavior, not a guaranteed diagnosis.

4. **The gateway-relative statistics depend on having enough historical observations.**  
   Very new gateways or gateways with sparse telemetry may be harder to score reliably.

5. **The network-change experiment is controlled.**  
   It does not reproduce every possible real-world network change.

6. **The solution does not automatically retrain while answering a live query.**  
